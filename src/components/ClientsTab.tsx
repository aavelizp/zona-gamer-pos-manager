...
import { UserPlus, Search, User, Phone, FileText, Trash2, Edit2 } from "lucide-react";
...
export function ClientsTab() {
  ...
  // 👇 AQUÍ SE APLICA EL ORDEN ALFABÉTICO RESTRICTO (A-Z) 👇
  const sortedAndFilteredClients = useMemo(() => {
    let result = Array.isArray(members) ? [...members] : [];

    // 1. Filtrar si hay una búsqueda activa
    if (query.trim()) {
    ...
    // 2. Ordenar alfabéticamente de la A a la Z
  ...
  const handleCreate = () => {
    ...
    // Validar si el teléfono ya existe para evitar duplicados
          ...
          <table className="w-full text-sm text-left min-w-[600px]">
            ...
            <tbody className="divide-y divide-border/50">
              {sortedAndFilteredClients.length === 0 ? (
                <tr><td colSpan={4} className="p-8 text-center text-muted-foreground italic">No se encontraron clientes registrados.</td></tr>
              ) : (
                sortedAndFilteredClients.map((c) => (
                  <tr key={c.id} className="hover:bg-secondary/10 transition-colors">
                    ...
                    <td className="p-3 sm:p-4 flex justify-center gap-2">
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-primary hover:bg-primary/20" onClick={() => { setEditName(c.name); setEditPhone(c.phone || ""); setEditDoc(c.idDoc || ""); setEditOpen(c); }}><Edit2 className="h-4 w-4" /></Button>
