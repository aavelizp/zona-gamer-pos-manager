...
// 👈 RELOJ INDEPENDIENTE PARA EVITAR EL CRASH
function useLocalNow(intervalMs = 1000) {
...
function CustomerSearch({ name, idDoc, phone, setName, setIdDoc, setPhone }: any) {
  ...
  const results = useMemo(() => {
    const q = query.trim().toLowerCase(); if (!q) return members.slice(0, 8);
    ...
    <div className="space-y-2 border border-border rounded-md p-3 bg-background/40">
      ...
      {!creating && (
        <div ref={wrapRef} className="relative">
          ...
          {selected && (<p className="text-[10px] text-success mt-1">✓ {selected.name} · {Math.round(selected.totalMinutes / 60)}h</p>)}
        </div>
      )}
      {(creating || selected) && (
        <div className="space-y-2">
          ...
          {creating && name.trim() && phone.trim() && (<p className="text-[10px] text-success">✓ Se creará en el Club Gamer</p>)}
        </div>
      )}
    </div>
  );
...
export function MultiCheckoutDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const now = useLocalNow(); // 👈 USAMOS EL RELOJ SEGURO
  ...
  const [receipt, setReceipt] = useState<ReceiptData | null>(null); const [pendingFinalize, setPendingFinalize] = useState(false);

  useEffect(() => {
    if (open) { setSelectedIds([]); setMethod("full"); setFullPayMode("cash"); setCashUsd(""); setMobileBs(""); setCashBs(""); setMobileBank(""); setMobileRef(""); setBillReceived(""); setName(""); setIdDoc(""); setPhone(""); setReceipt(null); setPendingFinalize(false); }
  }, [open]);
  ...
  consoles.filter(c => selectedIds.includes(c.id)).forEach(c => {
     ...
     extrasAmount += cExtras; c.charges.forEach(ch => items.push({ name: `${c.name}: ${ch.label}`, qty: 1, price: ch.amount }));
  });
  ...
  const buildReceipt = (): ReceiptData => ({
    ts: Date.now(), rate, consoleName: consoles.filter(c => selectedIds.includes(c.id)).map(c => c.name).join(" + "), minutes: totalMinutes, timeAmount, items, total, method: finalMethod, cashUsd: resolvedCashUsd, mobileBs: resolvedMobileBs, cashBs: resolvedCashBs, customer: { name: name.trim() || "Consumidor Final", idDoc: idDoc.trim() || undefined, phone: phone.trim() || undefined },
  });

  const doFinalize = () => {
    finalizeMulti(selectedIds, {
      method: finalMethod, cashUsd: resolvedCashUsd, mobileBs: resolvedMobileBs, cashBs: resolvedCashBs, mobileBank: needsRef ? mobileBank : undefined, mobileRef: needsRef ? mobileRef : undefined, customer: method === "credit" ? name.trim() : undefined, customerInfo: name.trim() ? { name: name.trim(), idDoc: idDoc.trim() || undefined, phone: phone.trim() || undefined } : undefined, total, timeAmount, extrasAmount, totalMinutes, items
    });
  };
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          <div className="border border-border/40 rounded-md p-3 bg-secondary/10">
            <Label className="text-xs uppercase tracking-wider text-muted-foreground mb-2 block font-semibold">Selecciona consolas:</Label>
            <div className="grid grid-cols-2 gap-2">
              {activeConsoles.map(c => ( <label key={c.id} className={`flex items-center gap-2 p-2 border rounded-md cursor-pointer transition-colors ${selectedIds.includes(c.id) ? 'border-purple-500 bg-purple-500/10' : 'border-border bg-card hover:bg-muted/50'}`}> <input type="checkbox" checked={selectedIds.includes(c.id)} onChange={() => toggle(c.id)} className="accent-purple-500 w-4 h-4" /> <span className="text-sm font-semibold">{c.name}</span> </label> ))}
              {activeConsoles.length === 0 && <p className="text-xs text-muted-foreground col-span-2">No hay consolas ocupadas.</p>}
            </div>
          </div>
          {selectedIds.length > 0 && (
            <>
              <Card className="p-3 bg-secondary/40"><div className="flex justify-between text-sm"><span>Tiempos Unidos</span><span>{fmtUsd(timeAmount)}</span></div><div className="flex justify-between text-sm"><span>Extras y Snacks</span><span>{fmtUsd(extrasAmount)}</span></div><div className="border-t border-border my-2" /><div className="flex justify-between font-display text-lg"><span>TOTAL GLOBAL</span><span className="text-purple-400">{fmtUsd(total)}</span></div><div className="flex justify-between text-sm text-accent"><span>En Bs</span><span>{fmtBs(total, rate)}</span></div></Card>
        ...
        <DialogFooter className="flex-wrap gap-2"><Button variant="outline" onClick={onClose}>Cancelar</Button><Button onClick={submit} disabled={selectedIds.length === 0 || (method === "mixed" && remaining > 0.01) || (method === "credit" && !name.trim()) || !isValidRef} className="bg-gradient-to-r from-purple-600 to-primary text-white"><Receipt className="h-4 w-4 mr-1" />Confirmar Cobro</Button></DialogFooter>
      </DialogContent>
    </Dialog>
