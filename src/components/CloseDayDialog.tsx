...
import { Gamepad2, ShoppingBag, Banknote, Smartphone, Handshake, FileText, AlertTriangle, Image as ImageIcon, FileSpreadsheet } from "lucide-react";
...
export function CloseDayDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  // 👇 ESTADO PARA MOSTRAR LA FOTO EN PANTALLA 👇
  ...
  // 👇 TÉCNICA INFALIBLE: GENERAR FOTO Y MOSTRARLA EN PANTALLA 👇
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      setPreviewImage(dataUrl); // Mostramos la imagen generada
  ...
  return (
    <>
      <Dialog open={open && !previewImage} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto p-0 bg-white">
          ...
          <div className="p-4 bg-slate-50 border-t border-slate-200 space-y-3 rounded-b-lg">
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white" onClick={handleGenerateImage}>
      ...
      {/* 👇 VENTANA FLOTANTE CON LA FOTO LISTA PARA GUARDAR 👇 */}
      {previewImage && (
        <Dialog open={!!previewImage} onOpenChange={(o) => !o && setPreviewImage(null)}>
          <DialogContent className="max-w-md bg-slate-100 p-4 rounded-xl">
            <DialogHeader>
              <DialogTitle className="text-center font-display text-slate-900">Captura Lista</DialogTitle>
            </DialogHeader>
            
            <div className="bg-amber-100 text-amber-800 p-3 rounded-md text-sm text-center border border-amber-200 mb-2">
              👆 <strong>Mantén presionada</strong> la imagen de abajo y selecciona <strong>"Guardar imagen"</strong> o <strong>"Compartir"</strong>.
            </div>

            <div className="rounded-lg overflow-hidden border-2 border-slate-300 shadow-lg bg-white flex justify-center items-center p-2">
              <img src={previewImage} alt="Cierre de Caja" className="max-w-full h-auto rounded" />
            </div>

            <DialogFooter className="mt-4">
              <Button onClick={() => setPreviewImage(null)} className="w-full bg-slate-900 text-white hover:bg-black">
                Entendido, cerrar
              </Button>
            </DialogFooter>
          </DialogContent>
