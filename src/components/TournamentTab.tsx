...
import { useStore, fmtUsd, type Tournament, type TournamentParticipant, type Member, type TournamentMatch } from "@/lib/store";
...
import { Trophy, Users, Receipt, Swords, ArrowLeft, Trash2, Plus, Play, UserPlus, Gift, FileText, Settings, Edit2, MessageCircle, X, Search, Table2, RotateCcw, Monitor } from "lucide-react";
...
export function TournamentTab() {
  ...
  const assignConsoleToMatch = useStore((s) => s.assignConsoleToMatch); 
  ...
  const [tFormat, setTFormat] = useState<"single_elimination" | "double_elimination" | "league" | "groups">("double_elimination"); 
  const [tMatchFormat, setTMatchFormat] = useState<"FT2" | "FT3" | "FT5">("FT2"); 
  ...
  const handleCreate = () => {
    ...
    createTournament({
      name: tName.trim(), 
      game: tGame.trim(), 
      maxPlayers: parseInt(tMax) || 999,
      entryFee: parseFloat(tFee) || 0, 
      prizePercentage: parseFloat(tPrize) || 50,
      format: tFormat, 
      groupCount: tFormat === "groups" ? parseInt(tGroupCount) : undefined,
      dateRange: new Date().toLocaleDateString('es-VE'),
      defaultMatchFormat: tMatchFormat
    });
    setCreateOpen(false); setTName(""); setTGame(""); setTFormat("double_elimination"); setTMatchFormat("FT2"); setTMax("999"); setTGroupCount("2"); setTFee("5"); setTPrize("50");
  ...
  const handleSaveScore = () => {
    ...
    if (scoreMatch.phase === "knockout" || activeTourney?.format === "single_elimination" || activeTourney?.format === "double_elimination") {
       if (s1 === s2) {
          toast.error("En fase eliminatoria debe haber un ganador.");
          return;
       }
    }
  ...
  const handleWalkover = (playerIndex: 1 | 2) => {
    if (!scoreMatch || !activeTourney) return;
    
    // Determinar puntos según el formato de la partida o el por defecto del torneo
    const format = scoreMatch.matchFormat || activeTourney.defaultMatchFormat || "FT2";
    let scoreToWin = 2; // Por defecto FT2
    if (format === "FT3") scoreToWin = 3;
    if (format === "FT5") scoreToWin = 5;

    const s1 = playerIndex === 1 ? scoreToWin : 0;
    const s2 = playerIndex === 2 ? scoreToWin : 0;

    setMatchScore(scoreMatch.id, s1, s2);
    setScoreMatch(null);
    toast.success(`Walkover aplicado al Jugador ${playerIndex === 1 ? 2 : 1}`);
  };
  ...
  const renderMatchCard = (m: TournamentMatch) => {
    ...
    return (
      <div key={m.id} className={`border rounded-md p-2 flex flex-col gap-1 relative shadow-sm ${m.winnerId ? 'border-primary/50 bg-primary/5' : 'border-border/60 bg-background'}`}>
        ...
        {/* Asignador de Consola - Si el partido está pendiente y ambos jugadores están listos */}
        {!m.winnerId && m.player1Id && m.player2Id && (
           <div className="mb-2">
             <div className="flex items-center gap-1 text-[10px] text-muted-foreground mb-1"><Monitor className="h-3 w-3" /> Asignar a consola:</div>
             <select 
               className="w-full h-7 rounded border border-border bg-secondary/50 px-2 text-xs focus:ring-1 focus:ring-primary outline-none cursor-pointer"
               value={m.assignedConsoleId || ""}
               onChange={(e) => assignConsoleToMatch(m.id, e.target.value)}
             >
  ...
  const renderBracketSection = (matchesArray: TournamentMatch[], title: string, icon: React.ReactNode, colorClass: string) => {
      if (matchesArray.length === 0) return null;
      return (
        <div className="mt-8 border border-border/40 bg-secondary/5 rounded-xl p-4 sm:p-6 shadow-sm overflow-x-auto">
          <h3 className={`font-display text-sm sm:text-base tracking-wider flex items-center gap-2 mb-6 ${colorClass}`}>
             {icon} {title}
          </h3>
          <div className="min-w-[800px] flex gap-8">
            {Array.from(new Set(matchesArray.map(m => m.round))).sort((a,b) => a - b).map(round => {
                const rMatches = matchesArray.filter(m => m.round === round).sort((a,b) => a.matchIndex - b.matchIndex);
                return (
      ...
      <div className="space-y-6">
        ...
        {tournaments.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {tournaments.map(t => {
              ...
              const formatLabel = t.format === "league" ? "Liga" : t.format === "groups" ? "Grupos" : t.format === "double_elimination" ? "Doble Eliminación" : "Eliminatoria";
              return (
                <Card key={t.id} className="p-4 border-border/40 hover:border-purple-500/50 transition-colors cursor-pointer" onClick={() => setActiveId(t.id)}>
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1 mb-2"><Swords className="h-3 w-3" /> {t.game} ({formatLabel}) {t.defaultMatchFormat && `· ${t.defaultMatchFormat}`}</p>
        ...
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-4">
              <div><Label>Nombre del Torneo</Label><Input value={tName} onChange={e => setTName(e.target.value)} placeholder="Ej: Kumite GamerZone" /></div>
              <div><Label>Videojuego</Label><Input value={tGame} onChange={e => setTGame(e.target.value)} placeholder="Ej: Mortal Kombat 1, Tekken 8..." /></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Formato</Label>
                  <select className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm focus:ring-1 focus:ring-primary" value={tFormat} onChange={e => setTFormat(e.target.value as any)}>
                    ...
                    <option value="double_elimination">Doble Eliminación (MK)</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Reglas (Peleas)</Label>
                  <select className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm focus:ring-1 focus:ring-primary" value={tMatchFormat} onChange={e => setTMatchFormat(e.target.value as any)}>
                    <option value="FT2">FT2 (Mejor de 3)</option>
                    <option value="FT3">FT3 (Mejor de 5)</option>
                    <option value="FT5">FT5 (Mejor de 9)</option>
                  </select>
                </div>
  ...
  // Separación de Llaves para Doble Eliminación
  const winnersMatches = kMatches.filter(m => m.bracket === "winners" || (!m.bracket && activeTourney.format !== 'double_elimination'));
  const losersMatches = kMatches.filter(m => m.bracket === "losers");
  const grandFinalsMatches = kMatches.filter(m => m.bracket === "grand_finals");
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{activeTourney.game} {activeTourney.defaultMatchFormat && `[Regla: ${activeTourney.defaultMatchFormat}]`} · {activeParts.length}{activeTourney.maxPlayers !== 999 ? `/${activeTourney.maxPlayers}` : ""} Inscritos</p>
          </div>
        </div>
      ...
      {activeTourney.status !== "registering" && (
        <div>
          {/* Fases Finales Separadas en Llaves */}
          {(activeTourney.format === "single_elimination" || activeTourney.format === "double_elimination" || kMatches.length > 0) && (
            <div className="space-y-4">
               {renderBracketSection(winnersMatches, activeTourney.format === "double_elimination" ? "Winners Bracket" : "Bracket Principal", <Trophy className="h-4 w-4" />, "text-gold")}
               {renderBracketSection(losersMatches, "Losers Bracket", <RotateCcw className="h-4 w-4" />, "text-red-400")}
               {renderBracketSection(grandFinalsMatches, "Grand Finals", <Trophy className="h-5 w-5" />, "text-purple-400")}
            </div>
          )}
        </div>
      )}

      {/* Tabla de Participantes (Se mantiene igual que antes) */}
      ...
      {/* MODAL DE RESULTADOS ACTUALIZADO CON W.O. Y REGLA DE ORO */}
      <Dialog open={!!scoreMatch} onOpenChange={(o) => !o && setScoreMatch(null)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/50 p-2 mt-2 rounded text-xs text-yellow-500 text-center font-semibold tracking-wide">
            ⚠️ Regla de Oro: El ganador de la partida NO puede cambiar de personaje. El perdedor SÍ puede.
          </div>
          ...
          <DialogFooter className="flex-col gap-2">
            <Button onClick={handleSaveScore} className="w-full bg-green-600 hover:bg-green-700 text-white h-12 text-lg">Guardar Resultado</Button>
            <div className="grid grid-cols-2 gap-2 mt-2">
                <Button variant="outline" size="sm" className="text-xs border-red-500/30 text-red-400 hover:bg-red-500/10" onClick={() => handleWalkover(1)}>W.O. Gana J1</Button>
                <Button variant="outline" size="sm" className="text-xs border-red-500/30 text-red-400 hover:bg-red-500/10" onClick={() => handleWalkover(2)}>W.O. Gana J2</Button>
            </div>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      {/* Resto de modales (Inscribir, etc.) */}
