...
interface State {
  ...
  payCredit: (creditId: string, payload: { method: PaymentMethod; cashUsd: number; mobileBs: number; mobileBank?: string; amount: number; customerInfo?: CustomerInfo }) => void;
...
export const useStore = create<State>()(
  persist(
    (set, get) => ({
      ...
      // 👇 ETIQUETAS DE GARANTÍA AGREGADAS EN LOS ESTADOS DEL TORNEO 👇
      ...
      enrollParticipant: (tournamentId, memberName, isPaid, payload) => set((s) => { const t = (s.tournaments||[]).find(x => x?.id === tournamentId); if (!t) return s; const participantId = uid(); let enrollSaleId = undefined; let newSales = s.sales || []; if (isPaid && payload) { enrollSaleId = uid(); const sale: SaleRecord = { id: enrollSaleId, ts: Date.now(), timeAmount: 0, extrasAmount: payload.total, total: payload.total, cashUsd: payload.cashUsd, mobileBs: payload.mobileBs, mobileBank: payload.mobileBank, mobileRef: payload.mobileRef, cashBs: payload.cashBs || 0, rate: s.rate, method: payload.method, customer: memberName, concept: `Inscripción Torneo: ${t.name}`, items: [{ name: `Inscripción: ${t.name}`, qty: 1, price: payload.total }] }; newSales = [...newSales, sale]; } const p: TournamentParticipant = { id: participantId, tournamentId, memberName, paymentStatus: isPaid ? ("paid" as "paid") : ("pending" as "pending"), enrolledAt: Date.now(), enrollSaleId }; return { participants: [...(s.participants||[]), p], sales: newSales }; }),
