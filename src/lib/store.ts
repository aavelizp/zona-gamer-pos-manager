...
// ============================================================================
// 🛡️ MOTOR DE RESCATE BLINDADO: ASEGURA QUE ZUSTAND NO VOMITE TUS DATOS 🛡️
// ============================================================================
const safeFormatData = (rawPayload: any) => {
  if (!rawPayload) return null;
  let core = rawPayload;
  
  // Extrae los datos reales sin importar cuántas cajas de "state" haya
  ...
  // Previene que arreglos faltantes causen un crasheo visual
  ...
  // Lo empaqueta EXACTAMENTE como Zustand lo exige: { state: {...}, version: 0 }
...
export const useStore = create<State>()(
  persist(
    (set, get) => ({
      ...
      enrollParticipant: (tournamentId, memberName, phone, isPaid, payload) => set((s) => { const t = (s.tournaments||[]).find(x => x?.id === tournamentId); if (!t) return s; const participantId = uid(); let enrollSaleId = undefined; let newSales = s.sales || []; if (isPaid && payload) { enrollSaleId = uid(); const sale: SaleRecord = { id: enrollSaleId, ts: Date.now(), timeAmount: 0, extrasAmount: payload.total, total: payload.total, cashUsd: payload.cashUsd, mobileBs: payload.mobileBs, mobileBank: payload.mobileBank, cashBs: payload.cashBs || 0, rate: s.rate, method: payload.method, customer: memberName, concept: `Inscripción Torneo: ${t.name}`, items: [{ name: `Inscripción: ${t.name}`, qty: 1, price: payload.total }] }; newSales = [...newSales, sale]; } const p: TournamentParticipant = { id: participantId, tournamentId, memberName, phone, paymentStatus: isPaid ? ("paid" as "paid") : ("pending" as "pending"), enrolledAt: Date.now(), enrollSaleId }; return { participants: [...(s.participants||[]), p], sales: newSales }; }),
      ...
      generateKnockoutFromGroups: (tournamentId) => set((s) => {
        ...
        for(let r = 1; r <= totalRounds; r++) { 
            ...
            for(let i = 0; i < matchesInRound; i++) { 
                ...
                koMatches.push({ id: matchId, tournamentId, round: r, matchIndex: i, player1Id: undefined, player2Id: undefined, winnerId: undefined, nextMatchId: nextRoundMatchId, phase: "knockout", matchFormat: t.defaultMatchFormat || "FT2" }); 
            } 
        } 
      ...
      setMatchScore: (matchId, score1, score2) => set((s) => {
        ...
        const loserId = m.player1Id === winnerId ? m.player2Id : (m.player2Id === winnerId ? m.player1Id : undefined);
        ...
        if (m.loserGoesToMatchId && loserId) {
            newMatches = newMatches.map(x => {
                if (x?.id === m.loserGoesToMatchId) {
                     if (!x.player1Id) return { ...x, player1Id: loserId };
                     if (!x.player2Id) return { ...x, player2Id: loserId };
                }
                return x;
            });
        }
        ...
        if (!m.nextMatchId && (!m.loserGoesToMatchId || m.bracket === "grand_finals")) {
      ...
      setMatchWinner: (matchId, winnerId) => set((s) => { 
        ...
        let newMatches = (s.matches||[]).map(x => x?.id === matchId ? { ...x, winnerId, isDraw: false, score1: undefined, score2: undefined } : x); 
        ...
        if (!m.nextMatchId && (!m.loserGoesToMatchId || m.bracket === "grand_finals")) { 
            ...
            if (t && t.format === "league") { 
                const allDone = newMatches.filter(x => x.tournamentId === m.tournamentId).every(x => x.winnerId || x.isDraw); 
                if (allDone) newTournaments = newTournaments.map(x => x?.id === m.tournamentId ? { ...x, status: "completed" as "completed" } : x); 
            } else if (t && t.format === "groups") { 
                if (m.phase === "knockout") { newTournaments = newTournaments.map(x => x?.id === m.tournamentId ? { ...x, status: "completed" as "completed" } : x); } 
            } else if (t) { 
                newTournaments = newTournaments.map(x => x?.id === m.tournamentId ? { ...x, status: "completed" as "completed" } : x); 
            } 
        } 
      ...
      revertMatchWinner: (matchId) => set((s) => { 
        ...
        const oldLoserId = m.player1Id === oldWinnerId ? m.player2Id : (m.player2Id === oldWinnerId ? m.player1Id : undefined); 
        ...
        if (m.loserGoesToMatchId && oldLoserId) {
            newMatches = newMatches.map(x => {
                if (x?.id === m.loserGoesToMatchId) {
                    if (x.player1Id === oldLoserId) return { ...x, player1Id: undefined };
                    if (x.player2Id === oldLoserId) return { ...x, player2Id: undefined };
                }
                return x;
            });
        }
        ...
        getItem: async (name) => { 
          try { 
            ...
            if (!error && data && data.state) { 
              // 🚨 AQUÍ USAMOS EL FORMATEADOR BLINDADO QUE OBLIGA A ZUSTAND A ACEPTAR TUS DATOS
              const safeData = safeFormatData(data.state); 
          ...
          if (local) { 
            try { return safeFormatData(JSON.parse(local)); } catch(e) {} 
          } 
          return null; 
        },
...
const resyncFromCloud = async () => { 
  ...
  try { 
    ...
    if ((window as any).pausarDescarga) return; 
    if (!error && data && data.state) { 
      // 🚨 AQUÍ TAMBIÉN USAMOS EL FORMATEADOR BLINDADO
      const safeData = safeFormatData(data.state); 
