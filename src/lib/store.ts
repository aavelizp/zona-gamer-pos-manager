...
interface State {
  ...
  createTournament: (t: Omit<Tournament, "id" | "createdAt" | "status">) => void; updateTournamentPrize: (id: string, newPercentage: number) => void; deleteTournament: (id: string) => void; enrollParticipant: (tournamentId: string, memberName: string, isPaid: boolean, payload?: any) => void; removeParticipant: (participantId: string) => void; payEnrollment: (participantId: string, payload: any) => void; generateBracket: (tournamentId: string) => void; setMatchWinner: (matchId: string, winnerId: string) => void; revertMatchWinner: (matchId: string) => void; revertTournamentToRegistering: (tournamentId: string) => void;
}
...
export const useStore = create<State>()(
  persist(
    (set, get) => ({
      ...
      updateTournamentPrize: (id, newPercentage) => set((s) => ({ tournaments: (s.tournaments||[]).map(t => t?.id === id ? { ...t, prizePercentage: newPercentage } : t) })),
      ...
      enrollParticipant: (tournamentId, memberName, isPaid, payload) => set((s) => { const t = (s.tournaments||[]).find(x => x?.id === tournamentId); if (!t) return s; const participantId = uid(); let enrollSaleId = undefined; let newSales = s.sales || []; if (isPaid && payload) { enrollSaleId = uid(); const sale: SaleRecord = { id: enrollSaleId, ts: Date.now(), timeAmount: 0, extrasAmount: payload.total, total: payload.total, cashUsd: payload.cashUsd, mobileBs: payload.mobileBs, mobileBank: payload.mobileBank, mobileRef: payload.mobileRef, cashBs: payload.cashBs || 0, rate: s.rate, method: payload.method, customer: memberName, concept: `Inscripción Torneo: ${t.name}`, items: [{ name: `Inscripción: ${t.name}`, qty: 1, price: payload.total }] }; newSales = [...newSales, sale]; } const p: TournamentParticipant = { id: participantId, tournamentId, memberName, paymentStatus: isPaid ? "paid" : "pending", enrolledAt: Date.now(), enrollSaleId }; return { participants: [...(s.participants||[]), p], sales: newSales }; }),
